   ...
   if (hasX && !all0()) {
      for (unsigned i = 0; i < b.size(); ++i) {
         if (b[i] == V3_BVX) {
            ...
            for (unsigned i = 0, j = shift; i < j; ++i) if (V3_BV1 == (*this)[i]) setX(i);
            if (_size <= shift) break;
         }
      }
   }
...
V3BitVecX::bv_select(const V3BitVecX& f, const V3BitVecX& t) const {
   ...
   for (unsigned i = 0; i < f._size; ++i) {
      t_value = t[i];
      if (!op_eq(t_value, f[i])) bv_sel.setX(i);
      else if (V3_BVX == t_value) bv_sel.setX(i);
   }
   return bv_sel;
}
...
V3BitVecS::bv_and(const V3BitVecS& v1, const bool& inv1, const V3BitVecS& v2, const bool& inv2) {
   ...
   /*
   _data0 = ((inv1) ? v1._data1 : v1._data0) | ((inv2) ? v2._data1 : v2._data0);
   _data1 = ((inv1) ? v1._data0 : v1._data1) & ((inv2) ? v2._data0 : v2._data1);
   */
}
